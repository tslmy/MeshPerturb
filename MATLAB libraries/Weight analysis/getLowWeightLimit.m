function [weight_low weights bflags epsilon Amps] = getLowWeightLimit(model_num, parameters, units, mesh, data_for_Mesh, midPlaneCoordinates, data_for_MidPlane, h_plate, modes)
%To obtain the weight for smallest imperfection that leads to bifurcation
% 
...
% weight_low: Smallest weight that leads to bifurcation
...
%Note: search starts from highest value and moves down until the weight does not change
...
%Variables
minWeight=0;                            %Lower bound for weight search
maxWeight=0.05;                        %Upper bound for weight search
...
step=0.0002;
weight_low=maxWeight;                       %Weighting factor
...
while flag
    ...
    try
        ...
        %Based on amplitude
        bflag =  ((v_value(end)/h_plate)>(margin));
    ...
    if bflag
        weight_low=weight;                  %This is a guaranteed bifurcation weight value
        maxWeight=weight;
    else
        if run_number>1
            minWeight=weight;
    ...
    %Expand search space if intial weight is too low for convergence
