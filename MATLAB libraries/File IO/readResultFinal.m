function [result r_name bflag] = readResultFinal(nonlinearModel, modifiedMidPlane)
% Read results via post processing of non-linear analysis model: final epsilon value only
...
displacement=[];
for i=1:dim
    displacement=[displacement mphinterp(nonlinearModel, variable{i}, 'coord', midplane.coordinates', 'solnum','end')'];
end
...
%% Format result
result=[epsilon(end) period amp ec];
